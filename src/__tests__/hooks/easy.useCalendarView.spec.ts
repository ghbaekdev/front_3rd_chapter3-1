...
describe('초기 상태', () => {
  ...
  it('currentDate는 오늘 날짜인 "2024-10-01"이어야 한다', () => {
    const { result } = renderHook(() => useCalendarView());
    expect(result.current.currentDate.toISOString()).toBe(new Date().toISOString());
  });

  it('holidays는 10월 휴일인 개천절, 한글날이 지정되어 있어야 한다', () => {
    const { result } = renderHook(() => useCalendarView());
    expect(result.current.holidays).toHaveProperty('2024-10-03');
    expect(result.current.holidays).toHaveProperty('2024-10-09');
  });
});
