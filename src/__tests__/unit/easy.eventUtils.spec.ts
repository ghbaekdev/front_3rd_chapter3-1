...
describe('getFilteredEvents', () => {
  ...
  it('주간 뷰에서 2024-11-01 주의 이벤트만 반환한다', () => {
  ...
  it('월간 뷰에서 2024년 11월의 모든 이벤트를 반환한다', () => {
    const result = getFilteredEvents(sampleEvents, '', new Date('2024-11-01'), 'month');
    expect(result).toHaveLength(3);
    expect(result.map((e) => e.id)).toEqual(['1', '2', '3']);
  });

  it("검색어 '이벤트'와 주간 뷰 필터링을 동시에 적용한다", () => {
    const result = getFilteredEvents(sampleEvents, '이벤트', new Date('2024-11-01'), 'week');
    expect(result).toHaveLength(2);
  ...
  it('월의 경계에 있는 이벤트를 올바르게 필터링한다', () => {
    const result = getFilteredEvents(sampleEvents, '', new Date('2024-10-31'), 'month');
    expect(result).toHaveLength(1);
